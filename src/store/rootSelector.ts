...
export const boardsGetBoardById = createSelector(
    boardsGetBoards,
    propsFirstSelector,
    (boards, idBoard) => boards.get(idBoard)
);

export const listState = (state: IStore) => state.lists;
export const listsGetListsByIdBoard = createSelector(
    listState,
    propsFirstSelector,
    (lists, idBoard) => lists.filter(list => list.idBoard === idBoard)
);

export const cardState = (state: IStore) => state.cards;
export const cardsGetCardsByIdList = createSelector(
    cardState,
    propsFirstSelector,
    (cards, idList) => cards.filter(card => card.idList === idList)
);
